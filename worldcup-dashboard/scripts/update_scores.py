#!/usr/bin/env python3
"""Update worldcup-dashboard/data/matches.json.
...
def normalize_football_data(raw: dict, current: dict) -> dict:
    # Keep a name lookup from existing seed data so Chinese display names can be preserved when possible.
    known = {(m.get("home"), m.get("away")): m for m in current.get("matches", [])}
    out = []
    for idx, m in enumerate(raw.get("matches", []), start=1):
...
def main() -> int:
    ...
    if not TOKEN:
        ...
        save(current)
        print("FOOTBALL_DATA_TOKEN not set; kept existing data.")
        return 0
    ...
    try:
        ...
        save(payload)
        print(f"Updated {len(payload.get('matches', []))} matches.")
        return 0
    except Exception as exc:
        print(f"Failed to update scores: {exc}", file=sys.stderr)
        return 1
